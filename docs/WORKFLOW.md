...
The Disagreer looks for material architecture/product problems, hidden assumptions, unnecessary complexity, missing acceptance criteria, simpler alternatives, and risky coupling.
...
The Fixer must never self-dispatch or create a recursive repair loop. If a blocker cannot be repaired safely, it reports `needs-human` and waits for human judgment or evidence.
